...
def init_db():
    ...
    # Note: Using specific Postgres syntax (SERIAL)
...
def get_df(query, params=None):
    """Fetch data using a SQL query and return a Pandas DataFrame."""
...
if check_password():
    ...
    active_jobs_df = get_df(
        "SELECT count(*) as count FROM schedule WHERE start_time <= :now AND end_time >= :now", 
        params={"now": now_str}
    )
        ...
        with col1:
            with st.form("add_robot"):
                ...
                if st.form_submit_button("Add Robot") and r_name:
                    # FIXED: Use :name syntax and dictionary params
                    run_query(
                        "INSERT INTO robots (name, model) VALUES (:name, :model)", 
                        {"name": r_name, "model": r_model}
                    )
                    st.rerun()
        with col2:
    ...
    # --- 3. Manage Operators (With Skills) ---
        ...
        # Get existing robot models safely
        try:
        ...
        with col1:
            with st.form("add_op"):
                ...
                if st.form_submit_button("Add Operator") and o_name:
                    ...
                    # FIXED: Use :name syntax and dictionary params
                    run_query(
                        "INSERT INTO operators (name, role, qualified_models) VALUES (:name, :role, :skills)", 
